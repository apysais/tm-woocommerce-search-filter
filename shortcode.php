 ...
 * Land Search
 */
function tm_search_land_ui_func( $atts ) {
	global $wp, $post;
	...
	wp_enqueue_script( tmwsf_get_text_domain(), $js_path . 'js/tm-woocommerce-search-land-filter-public.js', array( 'jquery' ), TM_WOOCOMMERCE_SEARCH_FILTER_VERSION, true );
	// Localize the script with new data
	$tm_arr_js = array(
			'home_url' => home_url( $wp->request ),
			'use_stickey_sidebar' => $atts['stickey_sidebar_class'] ? 1 : 0,
	...
	ob_start();
	require_once tmwsf_get_plugin_dir() . 'public/partials/search-land-ui.php';
	return ob_get_clean();
}
add_shortcode( 'tm_search_land_ui', 'tm_search_land_ui_func' );
